...
if [ "$MINIMAL_DMENU_TOOLS_GENERAL" != "true" ]; then
  ...
  log "Installing power-menu:" "$LOG_FILE"

  mkdir -p "$MDT_INSTALL_DIR"

  msg 'Welcome to the power-menu Installation Menu for "Minimal D-menu Tools!"'
...
case "$SELECTED_DMENU" in
    "bemenu") 
      log "power-menu installation succed!" "$LOG_FILE"
      ;;

    *) 
      log "power-menu installation succed! Please fix:" "$LOG_FILE"
      ...
      msg "[Theme] in line 9 of $POWER_INSTALL_DIR/power-menu.sh" "$LOG_FILE"
      msg "[Flag] in line 16 of $POWER_INSTALL_DIR/power-menu.sh" "$LOG_FILE"
      ;;
esac
