...
if [ "$MINIMAL_DMENU_TOOLS_GENERAL" != "true" ]; then
  ...
  log "Installing audio-menu:"

  mkdir -p "$MDT_INSTALL_DIR"

  echo 'Welcome to the audio-menu Installation Menu for "Minimal D-menu Tools!"'
...
case "$SELECTED_DMENU" in
    "bemenu") 
      log "audio-menu installation succed!" 
      ;;
    *) 
      log "audio-menu installation succed! Please fix:"
      ...
      echo "[Theme] in line 9 of $AUDIO_INSTALL_DIR/audio-menu.sh"
      echo "[Flag] in line 17 of $AUDIO_INSTALL_DIR/audio-menu.sh"
      ;;
esac
